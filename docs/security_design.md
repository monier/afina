...
- **Permissions**: **Restricted Access**.
  ...
  - _Note_: Can read encrypted blobs for backup/export purposes, but cannot modify them.
...
- Data sent to the Encryption Service is plain text _over the wire_ (protected by TLS), so the connection MUST be secure.
...
## Handler Boundary & Security

Authentication (login, refresh, MFA setup/verify) and API key validation are executed within dedicated handlers inside the Users slice and invoked via the custom mediator. Endpoints are thin and only perform binding + delegate. This ensures:

1. Centralized enforcement of auth, role, and tenant checks.
2. Consistent audit hooks (future pipeline behaviors) without duplicating logic in endpoints.
3. Avoidance of circular dependencies by restricting cross-slice interaction to mediator calls.

Example endpoint delegation:
